...
//	pDirector->enableRetinaDisplay(true);
	...
	if (cocos2d_utils::isIPhone5())
		CCEGLView::sharedOpenGLView()->setDesignResolutionSize(320, 568,
	        kResolutionShowAll);
	else if (cocos2d_utils::isIPhone())
	{
        CCEGLView::sharedOpenGLView()->setDesignResolutionSize(320, 320.0 * cocos2d_utils::getScreenAspectRatio(), //480,
			kResolutionShowAll);
    }
	else
    ...
    CCLOG("(PIRATES) nearest device size: %d, iphone aspect: %s, aspect ratio: %f",
        cocos2d_utils::getNearestDeviceSize(),
        cocos2d_utils::isIPhone() ? "true" : "false",
        cocos2d_utils::getScreenAspectRatio());

	if (cocos2d_utils::isIPhone())
		pDirector->setContentScaleFactor(cocos2d_utils::getNearestDeviceSize());
	else
		pDirector->setContentScaleFactor(cocos2d_utils::getNearestDeviceSize() / 2);
	...
	CCSprite* default_png = CCSprite::create(cocos2d_utils::createWithFormat("Default-%d.jpg",
	    cocos2d_utils::getNearestDeviceSize()).c_str());
		//cocos2d_utils::defaultPNGFileName().c_str());
	if (splashScreen->getContentSize().height / default_png->getContentSize().height >
        splashScreen->getContentSize().width / default_png->getContentSize().width)
    {
        default_png->setScale(splashScreen->getContentSize().height / default_png->getContentSize().height);
    }
    else
    {
        default_png->setScale(splashScreen->getContentSize().width / default_png->getContentSize().width);
    }

	default_png->setPosition(
			CCPoint(splashScreen->getContentSize().width * 0.5,
					splashScreen->getContentSize().height * 0.5));
	splashScreen->addChild(default_png);
	CCDirector::sharedDirector()->runWithScene(splashScreen);

	settings_.load();
	SoundManager::sharedSoundManager()->setIsSoundEnabled(
			settings_.isSoundEnabled());
	SoundManager::sharedSoundManager()->setIsMusicEnabled(
			settings_.isMusicEnabled());

	addTextureAsync("Menu-Background.jpg");
	addTextureAsync("Menu.png");
	addTextureAsync("Menu_Avatars.png");
	addTextureAsync("PlayScreen.png");
	addTextureAsync("Avatar_0.png");
	addTextureAsync("Avatar_1.png");
	addTextureAsync("Avatar_0_attack.png");
	addTextureAsync("Avatar_1_attack.png");
...
void AppDelegate::addTextureAsync(const char* path) {
	...
	std::string plist_file_name = cocos2d_utils::fileNameWithoutExtension(
			suffixed_path.c_str()) + ".plist";
	if (cocos2d_utils::isFileExists(
			CCFileUtils::sharedFileUtils()->fullPathForFilename(
					plist_file_name.c_str()))) {
...
void AppDelegate::loadNextTexture() {
	if (texturesToLoad_.size()) {
        ...
        CCLOG("(PIRATES) Loading: loadNextTexture: %s", suffixed_path.c_str());
		//std::cout << "loadNextTexture: " << suffixed_path << std::endl;

		CCTextureCache::sharedTextureCache()->addImageAsync(
				suffixed_path.c_str(), this,
				callfuncO_selector(AppDelegate::textureDidLoad));
	} else {
	    CCLOG("(PIRATES) Loading: All textures loaded");
		continueLoad();
	}
}
...
void AppDelegate::continueLoad() {
	...
	for (std::list<std::string>::const_iterator i = plistsToLoad_.begin(), e =
			plistsToLoad_.end(); i != e; ++i)
	{
	    CCLOG("(PIRATES) Loading: Load sprites from %s", i->c_str());
		CCSpriteFrameCache::sharedSpriteFrameCache()->addSpriteFramesWithFile(
				i->c_str());
	}

    CCLOG("(PIRATES) Create MainMenu");
	mainMenu_ = MainMenu::create();
	mainMenu_->retain();
	mainMenu_->setDelegate(this);

    CCLOG("(PIRATES) Create Settings");
...
//	onlineGameScreen_->setDelegate(this);
    ...
    CCLOG("(PIRATES) Create GameConfigurationScreen");
    ...
    CCLOG("(PIRATES) Create PlayScreen");
	...
	if (CCDirector::sharedDirector()->getRunningScene() != scene) {
		if (animated) {
			CCDirector::sharedDirector()->replaceScene(
					CCTransitionFade::create(0.3, scene, ccc3(0, 0, 0)));
		} else {
...
void AppDelegate::gameConfigurationScreenStartSinglePlayerGame(
		const PlayerConfig& player1Config, const PlayerConfig& aiConfig) {
	game_.startSinglePlayerGame(player1Config, aiConfig);
	//playScreen_->setOnlineGame(boost::shared_ptr<OnlineGame>());
	transitionToScreen(playScreen_);
}

void AppDelegate::gameConfigurationScreenStartTwoPlayersGame(
		const PlayerConfig& player1Config, const PlayerConfig& player2Config) {
...
void AppDelegate::gameConfigurationScreenStartOnlineGame(
		monsters::OnlineGame *game) {
...
void AppDelegate::onGetActiveOnlineGame(OnlineGame* onlineGame,
		const Error& error) {
...
void AppDelegate::onProcessInviteCompleted(OnlineGame* onlineGame,
		const Error& error) {
//	mainMenu_->hideLoadingIndicator();
//	
//	if (error.isOk())
