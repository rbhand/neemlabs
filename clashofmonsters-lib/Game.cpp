...
void Game::removeUnit(const Unit* unit)
{
	...
	assert(unit->player() == activePlayer_);
	//assert(!unit->isWall());
	assert(!unit->attackFormation());
	...
	bool attackPlayer)
{
	...
	while (units_to_attack.size())
	{
		...
		if (du->attackFormation_)
		{
			...
			assert(du->attackFormation()->strength() >= 0);
			assert(f->strength() >= 0);
...
//			}
			...
			assert(it != ap.attackFormations_.end());
...
{
	assert(players_[activePlayer_].turnsLeft_ >= 0);
