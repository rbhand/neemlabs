...
bool LargeUnitMenuItem::init(const char* spriteFrameName, UnitType* type, cocos2d::CCObject* target, SEL_MenuHandler selector)
{
	...
	unitDescription_ = CCLabelTTF::create(type->description().c_str(), "Myriad Pro", ct(11), CCSize(getContentSize().width - ct(5) - normal_sprite->getContentSize().width, 0), kCCTextAlignmentLeft);
	...
	unitUnlockDescription_ = CCLabelTTF::create( ("Unlock for " + cost_formatted + " XP").c_str(),
		"Myriad Pro", ct(18), CCSize(getContentSize().width - ct(5) - normal_sprite->getContentSize().width, 0), kCCTextAlignmentLeft);
