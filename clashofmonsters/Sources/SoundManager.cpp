...
{
	currentBGMusicName_ = CCString::createWithFormat("");
...
{
	currentBGMusicName_->release();
	currentBGMusicName_ = CCString::createWithFormat(path);
	currentBGMusicName_->retain();
...
unsigned int SoundManager::playEffect(const char* effect_name, bool loop)
{
	...
	if (isSoundEnabled_)
		return SimpleAudioEngine::sharedEngine()->playEffect(CCString::createWithFormat("Sound/%s.caf", effect_name)->getCString(), loop);
