	...
	for (NSString* file_name in files)
	{
			...
			[[file_name pathExtension] isEqualToString:@"jpeg"])
		{
			...
			if (image)
			{
				...
				frame->absoluteDirPath = [dir_path_string UTF8String];
				...
				int width = frame->fullImageSize.width = (int)CGImageGetWidth(image);
				int height = frame->fullImageSize.height = (int)CGImageGetHeight(image);
...
void ssg::write_cropped_frame(const Frame& frame, const ssg::rect_t& rect, const std::string& path)
{
	// Create a bitmap graphics context of the given size
    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
	unsigned char *rawData = (unsigned char*)calloc(rect.height * rect.width * 4, sizeof(unsigned char));
    CGContextRef context = CGBitmapContextCreate(rawData, rect.width, rect.height, 8, 4 * rect.width,
	...
	CGImageRef image = [[img_path pathExtension] isEqualToString:@"png"] ?
		CGImageCreateWithPNGDataProvider(image_data, NULL, true, kCGRenderingIntentDefault) :
		CGImageCreateWithJPEGDataProvider(image_data, NULL, true, kCGRenderingIntentDefault);
	CGImageRef partial_image = CGImageCreateWithImageInRect(image, CGRectMake(rect.x, rect.y,
		rect.width, rect.height));
	CGContextDrawImage(context, CGRectMake(0, 0, rect.width, rect.height), partial_image);
